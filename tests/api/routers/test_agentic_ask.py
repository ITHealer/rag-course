...
class TestAgenticAskEndpoint:
    ...
    def test_ask_agentic_returns_citations_payload(self, client, mock_agentic_rag_service):
        """Test structured citation fields in response payload."""
        mock_agentic_rag_service.ask = AsyncMock(return_value={
            "query": "test",
            "answer": "grounded answer",
            "sources": [{"source_uri": "file://doc.pdf"}],
            "citations": [
                {
                    "ref": 1,
                    "source_type": "project",
            ...
            "source_count": 1,
            "reasoning_steps": ["planner", "retrieve", "answer"],
            "retrieval_attempts": 1,
            "rewritten_query": None,
            "preset_id": "scoped_knowledge",
        })

        response = client.post("/api/v1/ask-agentic", json={"query": "test"})

        assert response.status_code == 200
        payload = response.json()
        assert payload["source_count"] == 1
        assert len(payload["citations"]) == 1
        assert payload["preset_id"] == "scoped_knowledge"
