...
from src.services.tracing.langfuse_tracer import RAGTracer
...
async def _prepare_chunks_and_sources(
    ...
    with rag_tracer.trace_search(trace, request.query, request.top_k) as search_span:
        ...
        search_mode = search_results.get("search_mode", "bm25")
        search_error = search_results.get("error")
        if search_error:
            logger.warning(f"Search degraded for query '{request.query}': {search_error}")
    ...
    return chunks, list(sources_set), arxiv_ids, search_mode
