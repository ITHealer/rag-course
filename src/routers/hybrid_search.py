...
async def hybrid_search(
    ...
    try:
        ...
        hybrid_requested = request.use_hybrid and query_embedding is not None
        logger.info(f"Hybrid search: '{request.query}' (hybrid: {hybrid_requested})")
        ...
        if results.get("degraded"):
            logger.warning(f"Search returned degraded mode fallback for query '{request.query}': {results.get('error')}")
