...
class IngestDebugSettings(BaseConfigSettings):
    model_config = SettingsConfigDict(
        env_file=[".env", str(ENV_FILE_PATH)],
        env_prefix="INGEST_DEBUG__",
        extra="ignore",
        frozen=True,
        case_sensitive=False,
    )
    ...
    enabled: bool = False
    output_dir: str = "./debug_ingestion"
...
class Settings(BaseConfigSettings):
    ...
    preset_dir: str = "./src/presets"
    default_preset_id: str = "scoped_knowledge"
    project_upload_dir: str = "./data/projects"
    project_ingestion_max_concurrency: int = 2

    # Feature Toggles for Pluggable Architecture
    vector_db_provider: Literal["opensearch", "qdrant"] = "opensearch"
    embedding_provider: Literal["jina", "ollama", "sentence-transformers"] = "jina"
    tracing_provider: Literal["langfuse", "local"] = "langfuse"
    ...
    ingest_debug: IngestDebugSettings = Field(default_factory=IngestDebugSettings)
