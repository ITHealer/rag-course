...
class HybridIndexingService:
    ...
    def __init__(self, chunker: TextChunker, embeddings_client: BaseEmbeddingsClient, opensearch_client: BaseVectorStore):
    ...
    def create_chunks(self, paper_data: Dict) -> List[TextChunk]:
        """Create chunks for a paper using the shared chunking path."""
    ...
    async def index_paper(self, paper_data: Dict, precomputed_chunks: Optional[List[TextChunk]] = None) -> Dict[str, int]:
        """Index a single paper with chunking and embeddings.

        :param paper_data: Paper data from database
        :param precomputed_chunks: Optional shared chunks generated upstream
        ...
        try:
            # Step 1: Chunk the paper using the shared chunking path
            chunks = precomputed_chunks if precomputed_chunks is not None else self.create_chunks(paper_data)
