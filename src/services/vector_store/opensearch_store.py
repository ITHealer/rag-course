...
from src.services.opensearch.index_config_hybrid import HYBRID_RRF_PIPELINE, get_hybrid_index_mapping
...
class OpenSearchStore(BaseVectorStore):
    ...
    def __init__(self, host: str, settings: Settings):
        ...
        self._schema_is_compatible: Optional[bool] = None
    ...
    def setup_indices(self, force: bool = False) -> Dict[str, bool]:
        ...
        self.assert_index_compatible(refresh=True)
        results["rrf_pipeline"] = self._create_rrf_pipeline(force)
        return results

    def validate_index_compatibility(self) -> Dict[str, Any]:
        """Validate existing index schema is compatible with hybrid search requirements."""
        issues: List[str] = []
        ...
        mapping = mapping_response.get(self.index_name, {}).get("mappings", {})
        properties = mapping.get("properties", {})

        settings_response = self.client.indices.get_settings(index=self.index_name)
        index_settings = settings_response.get(self.index_name, {}).get("settings", {}).get("index", {})
        ...
        if embedding_dimension != expected_dimension:
            issues.append(f"Field 'embedding' dimension expected '{expected_dimension}' but got '{embedding_dimension}'.")

        return {"is_compatible": len(issues) == 0, "issues": issues}

    def assert_index_compatible(self, refresh: bool = False) -> None:
        """Raise a typed error when index schema does not match hybrid requirements."""
        if self._schema_is_compatible is True and not refresh:
            return

        compatibility = self.validate_index_compatibility()
        if not compatibility["is_compatible"]:
            self._schema_is_compatible = False
            raise IncompatibleIndexSchemaError(index_name=self.index_name, issues=compatibility["issues"])

        self._schema_is_compatible = True

    def ensure_index_ready(self) -> None:
        """Ensure index exists and has compatible schema before write operations."""
        if not self.client.indices.exists(index=self.index_name):
            raise IndexNotReadyError(
                index_name=self.index_name,
                reason="Index does not exist. Run setup_indices() before indexing documents.",
            )
    ...
    def _create_hybrid_index(self, force: bool = False) -> bool:
        ...
        try:
            if force and self.client.indices.exists(index=self.index_name):
                ...
                self._schema_is_compatible = None
                logger.info(f"Deleted existing hybrid index: {self.index_name}")

            if not self.client.indices.exists(index=self.index_name):
                mapping = get_hybrid_index_mapping(dimension=self.settings.opensearch.vector_dimension)
                self.client.indices.create(index=self.index_name, body=mapping)
                self._schema_is_compatible = None
                logger.info(f"Created hybrid index: {self.index_name} with dimension {self.settings.opensearch.vector_dimension}")
                return True
    ...
    def search_unified(
        ...
        try:
        ...
        except IncompatibleIndexSchemaError as e:
            logger.error(f"Hybrid search disabled due to incompatible index schema: {e}")
            fallback_results = self._search_bm25_only(
                query=query,
                size=size,
                from_=from_,
            ...
            fallback_results["degraded"] = True
            ...
            fallback_results["degraded"] = True
            fallback_results["error"] = f"Hybrid search unavailable: {e}"
            return fallback_results
    ...
    def _search_bm25_only(
        ...
        results = {"total": response["hits"]["total"]["value"], "hits": [], "search_mode": "bm25", "degraded": False, "error": None}
    ...
    def _search_hybrid_native(
        ...
        results = {"total": response["hits"]["total"]["value"], "hits": [], "search_mode": "hybrid", "degraded": False, "error": None}
    ...
    def index_chunk(self, chunk_data: Dict[str, Any], embedding: List[float]) -> bool:
        ...
        except (IndexNotReadyError, IncompatibleIndexSchemaError):
            raise
        except Exception as e:
