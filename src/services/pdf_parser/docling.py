...
class DoclingParser:
    ...
    async def parse_pdf(self, pdf_path: Path) -> Optional[PdfContent]:
        ...
        try:
            ...
            logger.info(f"Document conversion finished. Items count: {len(doc.texts) if hasattr(doc, 'texts') else 'N/A'}")

            # Try to use export_to_text() first
            try:
                raw_text = doc.export_to_text()
            except Exception as e:
                logger.warning(f"export_to_text() failed: {e}. Trying export_to_markdown()")
                raw_text = doc.export_to_markdown()

            if not raw_text.strip():
                 logger.warning("Extracted raw_text is empty!")
