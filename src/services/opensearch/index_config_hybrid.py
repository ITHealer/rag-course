...
def get_hybrid_index_mapping(dimension: int = 1024) -> dict:
    """Get index mapping for chunked papers with vector embeddings.
    
    :param dimension: Vector dimension for k-NN search
...
def get_universal_project_index_mapping(dimension: int = 1024) -> dict:
    """Get universal project-scoped mapping for multi-domain grounded retrieval."""
    return {
        "settings": {
            "number_of_shards": 1,
            "number_of_replicas": 0,
...
# Keep legacy name for backward compatibility during refactor if needed
ARXIV_PAPERS_CHUNKS_MAPPING = get_hybrid_index_mapping()
